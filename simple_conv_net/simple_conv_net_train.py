...
class SimpleConvNetVectorized(nn.Module):
    def __init__(self, device):
        super(SimpleConvNetVectorized, self).__init__()
        self.device = device
        self.conv_layer = nn.Conv2d(in_channels=1,
                                    out_channels=20,
                                    kernel_size=5,
    ...
    def forward(self, x):
        ...
        z_fc1 = fc_layer_vector(
            z_pool_reshaped, self.fc_layer1.weight, self.fc_layer1.bias, self.device)
        z_relu = relu_vector(z_fc1, self.device)
        z_fc2 = fc_layer_vector(
            z_relu, self.fc_layer2.weight, self.fc_layer2.bias, self.device)
        y = F.softmax(z_fc2, dim=1)
        return y


class SimpleConvNetScalar(nn.Module):
    def __init__(self, device):
        super(SimpleConvNetScalar, self).__init__()
        self.device = device
        self.conv_layer = nn.Conv2d(in_channels=1,
                                    out_channels=20,
                                    kernel_size=5,
    ...
    def forward(self, x):
        ...
        z_fc1 = fc_layer_scalar(
            z_pool_reshaped, self.fc_layer1.weight, self.fc_layer1.bias, self.device)
        z_relu = relu_scalar(z_fc1, self.device)
        z_fc2 = fc_layer_scalar(
            z_relu, self.fc_layer2.weight, self.fc_layer2.bias, self.device)
        y = F.softmax(z_fc2, dim=1)
        return y
...
def main(args):
    ...
    model = SimpleConvNetVectorized(device)
